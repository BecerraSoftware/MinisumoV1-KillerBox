...
void loop() {
  if (digitalRead(4) == 1)
  {
    ...
    bool obstFor = DetectarObstaculo(delForward);
   ...
   if (pisoIzq || pisoDer) {
    ...
    // 🔹 Si detecta solo el sensor izquierdo → gira a la izquierda hasta verlo con el VL53X
    if (obstIzq && !obstDer) {
      ...
      while (millis() - startTime < 1000) { // Evita que se quede trabado
       ...
       // if (DetectaBlanco(floorLeft) || DetectaBlanco(floorRigh)) break; // Seguridad
        ...
        if (DetectarObstaculo(delForward)) { // Lo vio con el VL53X
          Avanzar(255, 0);
          delay(50);
          break;
        }
    ...
    // 🔹 Si detecta solo el sensor derecho → gira a la izquierda hasta verlo con el VL53X
    if (obstDer && !obstIzq) {
      unsigned long startTime = millis();
      while (millis() - startTime < 1000) {
        Avanzar(80, 2);  // 🔁 Gira a la IZQUIERDA (corregido)
      //  if (DetectaBlanco(floorLeft) || DetectaBlanco(floorRigh)) break;

        if (DetectarObstaculo(delForward)) { // Si ya lo ve el VL53X
          Avanzar(255, 0);
          delay(50);
          break;
        }
